...
CTDAttribMgr::CTDAttribMgr(LPCTSTR attributesFile, LPCTSTR nameFile, LPCTSTR supFile, bool bIncludeNonVA) 
    : m_attributesFile(attributesFile), m_nameFile(nameFile), m_supFile(supFile), m_bIncludeNonVA(bIncludeNonVA)
{
}
...
bool CTDAttribMgr::readAttributes()
{
    ...
    try {
        ...
        while (attribFile.ReadString(lineStr)) {
            ...
            if (attribName.CompareNoCase(TD_VID_ATTRIB_NAME) != 0 && attribName.CompareNoCase(TD_SENSITIVE_NAME) != 0) {
                // Create an attribute.
...
//---------------------------------------------------------------------------
// Write the name of the suppressed attribute to a file.
//---------------------------------------------------------------------------
bool CTDAttribMgr::writeSupFile()
{
    cout << _T("Writing suppressed attributes file...") << endl;
    try {
        CStdioFile supFile;
        if (!supFile.Open(m_supFile, CFile::modeCreate | CFile::modeWrite)) {
            cerr << _T("CTDAttribMgr: Failed to open file ") << m_supFile << endl;
            return false;
        }
        ...
        for (int a = 0; a < nAttribs; ++a) {
            ...
            for (POSITION pos = pAttrib->m_cut.GetHeadPosition(); pos != NULL;) {
                pCutConcept = pAttrib->m_cut.GetNext(pos);
                if (!pCutConcept->getParentConcept()) {
                    // This is the root, so it is suppressed.
                    supFile.WriteString(pAttrib->m_attribName);
                    supFile.WriteString(_T("\n"));
                    ASSERT(pAttrib->m_cut.GetCount() == 1);
                    break;
                }
            }
        }
        ...
        cerr << _T("Failed to write suppressed attributes file: ") << m_supFile << endl;
        ASSERT(false);
        return false;
    }
    cout << _T("Writing suppressed attributes file succeeded.") << endl;
    return true;
}
...
//---------------------------------------------------------------------------
bool CTDAttribMgr::pickSpecializeConcept(CTDAttrib*& pSelectedAttrib, CTDConcept*& pSelectedConcept)
{
    ...
    float maxInfoAnoyRatio = -1.0f;
#else
    // Specialize if score > 0.
    float maxInfoAnoyRatio = 0.0f;
#endif
    CTDAttrib* pAttrib = NULL;
    CTDConcept* pCutConcept = NULL;
    int nAttribs = m_attributes.GetSize() - 1;  // exclude class attribute
    for (int a = 0; a < nAttribs; ++a) {
        ...
        // Mode: not include non virtual attribute. So, never specialize on this attribute.
        if (!m_bIncludeNonVA && !pAttrib->m_bVirtualAttrib)
            continue;
        ...
        for (POSITION pos = pAttrib->m_cut.GetHeadPosition(); pos != NULL;) {
            ...
            if (pCutConcept->m_infoAnoyRatio > maxInfoAnoyRatio) {
                maxInfoAnoyRatio = pCutConcept->m_infoAnoyRatio;
                pSelectedAttrib = pAttrib;
                pSelectedConcept = pCutConcept;
            }
        }
    }
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
bool CTDAttribMgr::computeInfoGain()
{    
    // For each attribute.    
    CTDAttrib* pAttrib = NULL;
    CTDConcept* pCutConcept = NULL;
    ...
    for (int a = 0; a < nAttribs; ++a) {
        ...
        for (POSITION conPos = pAttrib->m_cut.GetHeadPosition(); conPos != NULL;) {
            ...
            if (!pCutConcept->m_bCutCandidate)
                continue;
            if (!pCutConcept->computeInfoGain(getNumClasses()))
                return false;
        }
    }
    return true;
