...
CTDAttrib::CTDAttrib(LPCTSTR attribName)
    : m_attribName(attribName), m_pConceptRoot(NULL), m_attribIdx(-1), m_nOFlatConcepts(-1), m_bVirtualAttrib(false), m_bSensAttrib(false), m_bMaskTypeSup(false)
{
    m_pFlattenConcepts = new CTDConcepts();
}

CTDAttrib::~CTDAttrib() 
{
    delete m_pFlattenConcepts;
    m_pFlattenConcepts = NULL;
...
bool CTDAttrib::flattenHierarchy()
{
    ...
    while (!attList.IsEmpty()) {
        ...
        pConcept->m_flattenIdx = m_pFlattenConcepts->Add(pConcept);
    ...
    m_nOFlatConcepts = m_pFlattenConcepts->GetSize();
    return true;
}

//---------------------------------------------------------------------------
// Check whether the given VID is related to this attribute.
//---------------------------------------------------------------------------
bool CTDAttrib::isRelated(CTDVirtualID* pVID)
{
    CTDAttribs* pVAttribs = pVID->getVirtualAttribs();
    int nVAttribs = pVAttribs->GetSize();
    for (int a = 0; a < nVAttribs; ++a) {
        if (pVAttribs->GetAt(a) == this) {
            return true;
        }
    }
    return false;
...
bool CTDDiscAttrib::initHierarchy(LPCTSTR conceptStr)
{
    ...
    for (int c = 0; c < m_pFlattenConcepts->GetSize(); ++c) {
        cout << m_pFlattenConcepts->GetAt(c)->m_conceptValue << _T(" ");
    }
    cout << endl;
#endif
    return true;
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
CTDAttrib* CTDAttribs::findAttribute(LPCTSTR target)
{
    CTDAttrib* pAttrib = NULL;
    for (int i = 0; i < GetSize(); ++i) {
        pAttrib = GetAt(i);
        if (pAttrib->m_attribName.CompareNoCase(target) == 0)
            return pAttrib;
    }
    return NULL;
}
